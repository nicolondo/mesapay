"""
One-pager comercial dirigido al cliente (restaurante) — versión México.

Salida: /Users/nicolas/Documents/APPS/MESAPAY/docs/MESAPAY-Planes-y-Precios-MX.pdf
...
OUTPUT = "/Users/nicolas/Documents/APPS/MESAPAY/docs/MESAPAY-Planes-y-Precios-MX.pdf"
...
def mxn(n: int) -> str:
    """Formato moneda MXN: separador de miles por coma (ej. $3,999)."""
    return "$" + f"{n:,.0f}"


# Datos de los planes — precios en MXN
PLANES = [
    {
        "nombre": "Esencial",
        "tagline": "Para 1 local, hasta 20 mesas",
        "precio_mensual": 1_999,
        "destacado": False,
        "features": [
...
def plan_card(plan: dict, width_cm: float) -> Table:
    ...
    # Precio mensual
    rows.append([Paragraph(f"{mxn(plan['precio_mensual'])}", style_price_big)])
    ...
    rows.append([Paragraph(
        f"{mxn(pa_mensualizado)}/mes <font size='8'>· pago anual</font>",
        style_price_annual,
    )])
    rows.append([Paragraph(
        f"({mxn(pa_total)} al año · ahorras {mxn(ahorro)})",
        style_price_save,
    )])
...
def build():
    ...
    doc = BaseDocTemplate(
        ...
        title="MESAPAY · Planes y precios · México",
        author="MESAPAY",
    )
    ...
    story.append(Paragraph("PLANES Y PRECIOS · MÉXICO · 2026", style_eyebrow))
    ...
    story.append(Paragraph(
        ...
        "el celular y pagan al final. Liberas meseros, aceleras la rotación y vendes "
    ...
    story.append(Paragraph(
        "Precios en MXN, no incluyen IVA. El plan se elige según el tamaño del restaurante. "
    ...
    story.append(Paragraph(
        "El cálculo es simple. Un mesero en México cuesta <b>~$12,000 MXN/mes</b> "
        "cuando sumas el salario base y las prestaciones de ley: IMSS, aguinaldo, "
        "prima vacacional y días de descanso. MESAPAY automatiza el pedido, el pago, "
        "la comanda a cocina y el cierre — el equivalente al trabajo de mínimo un mesero. "
    ...
    # Plan Pro: $3,999 MXN/mes. Anual 15% off: $3,999 * 0.85 = $3,399/mes equivalente
    pro_mensual = 3_999
    pro_anual_mens = precio_anual_mensualizado(pro_mensual, DESCUENTO_ANUAL)  # 3,399
    roi = Table([[
        Paragraph("<b>Restaurante de 30 mesas</b><br/>"
                  "Plan recomendado: <b>Pro</b><br/>"
                  f"Costo mensual: <b>{mxn(pro_mensual)}</b><br/>"
                  f"<font color='#6b6b6b' size='8'>Pago anual con 15% off: {mxn(pro_anual_mens)}/mes equivalente</font>",
                  style_body_l),
        Paragraph("<b>Lo que ganas</b><br/>"
                  "· Ahorro mínimo equivalente a 1 mesero (~$12,000 MXN/mes con prestaciones)<br/>"
                  ...
                  "· <b>ROI neto: ~$8,000 MXN/mes recuperados</b> (ahorro − costo del plan)",
                  style_body_l),
    ...
    data = [[
        ...
        Paragraph("Ahorras con promo", th_style),
    ]]
    for p in PLANES:
        pm = p["precio_mensual"]
        ...
        data.append([
            ...
            Paragraph(mxn(pm) + "/mes", td_r),
            Paragraph(mxn(pa15) + "/año", td_r),
            Paragraph(mxn(pa20) + "/año", td_rb),
            Paragraph(mxn(ahorro20), td_green),
        ])
    ...
    story.append(t)
    story.append(Paragraph(
        "Cifras en MXN, no incluyen IVA. Promo válida hasta el 31 de diciembre de 2026.",
        style_caption,
    ))
    ...
    cta = Table([[Paragraph(
        ...
        "<font color='#3a3a3a'>Habla con tu ejecutivo MESAPAY o escríbenos a "
