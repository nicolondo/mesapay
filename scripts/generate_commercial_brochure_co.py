#!/usr/bin/env python3
"""
Folleto comercial MESAPAY para restaurantes en Colombia (A4 vertical, 6 páginas).
Genera HTML y lo imprime a PDF con Chrome headless (mismo método que el brand kit).

Salida: docs/MESAPAY-Presentacion-Comercial-CO.pdf
"""
import base64
import os
import subprocess
...
LOGO_DARK = f"{ROOT}/brand-kit/_build/logo-white-on-ink.png"  # blanco, para fondos oscuros
LOGO_LIGHT = f"{ROOT}/brand-kit/_build/logo-on-bone.png"             # para fondos claros
BUILD_HTML = f"{ROOT}/brand-kit/_build/brochure_co.html"
OUTPUT = f"{ROOT}/docs/MESAPAY-Presentacion-Comercial-CO.pdf"
...
# ── íconos SVG (stroke terracota, 24x24) ──────────────────────────────
...
FEATURES = [
    ...
    (IC_CAL, "Reservas con depósito", "Recibí reservas online y cobrá un depósito para frenar los no-shows."),
    (IC_PHONE, "App del mesero", "Tus meseros toman pedidos, cobran y reciben avisos desde el celular."),
    (IC_DOC, "Factura electrónica DIAN", "Factura válida ante la DIAN, enviada al correo del cliente al instante."),
    (IC_CASH, "Cierre de turno y arqueo", "Cuadre de caja, propinas y reportes contables al cerrar el turno."),
    (IC_BUILDING, "Multi-sucursal", "Administrá varias sedes desde un solo panel, con datos consolidados."),
    (IC_AI, "Pulso · asistente de IA", "Preguntale a tus datos: platos más vendidos, horas pico, mejor mesero.", True),
]
...
PROBLEMS = [
    ("Meseros caros y escasos", "Un mesero cuesta cerca de <b>$3.000.000/mes</b> con prestaciones — y conseguirlos y retenerlos es cada vez más difícil."),
    ("Mesas que rotan lento", "El cliente espera para pedir, espera la cuenta y espera el datáfono. Cada minuto extra es una mesa que no girás."),
...
STEPS = [
    ...
    ("03", "Paga", "Paga desde el celular con tarjeta, PSE o Apple Pay, o divide la cuenta entre varios. Recibe su factura DIAN al correo."),
]
...
<style>
  ...
  * {{ box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }}
  ...
  /* ── Portada ── */
  .cover{{ background:var(--ink); color:var(--ivory); display:flex; flex-direction:column; }}
...
<section class="page cover">
  ...
  <div class="kick">Carta digital · Pedido y pago por QR · Para restaurantes en Colombia</div>
  <h1>Tu restaurante<br>vende más y<br>trabaja menos.</h1>
  <div class="lead">Tus comensales escanean, ordenan y pagan desde el celular. Vos liberás meseros,
    acelerás la rotación y subís el ticket promedio. Sin app, sin hardware costoso, en menos de una semana.</div>
  <div class="tagrow">
    <span class="tag">Sin app para el cliente</span>
    <span class="tag">Sin hardware costoso</span>
    <span class="tag">Factura DIAN</span>
    <span class="tag">Pagos reales (tarjeta · PSE)</span>
  </div>
  <div class="spacer"></div>
...
<section class="page">
  ...
  <div class="note"><b>Sin app para tus clientes. Sin equipos costosos.</b> Tu cliente usa el celular que ya
    tiene en la mano; de tu lado basta una tablet en cocina y los celulares de tus meseros — nada de POS
...
<section class="page">
  ...
  <div class="bignum">
    <div class="glow"></div>
    <div class="n">~$3.000.000<span style="font-size:22px;color:#D8CFBF">/mes</span></div>
    <div class="nlabel">Es lo que cuesta un mesero en Colombia, con prestaciones. MESAPAY automatiza el pedido,
      el pago, la comanda y el cierre — el equivalente al trabajo de al menos un mesero.</div>
  </div>
  <div class="example">
    <div class="eh"><div class="et">Restaurante de 30 mesas</div><div class="epill">Plan Pro</div></div>
    <ul class="elist">
      <li>Ahorro equivalente a <b>al menos 1 mesero</b> (~$3M/mes con prestaciones).</li>
      ...
      <li>Menos descuadres, menos errores de comanda y cierres de caja exactos.</li>
    </ul>
    <div class="roi"><span class="rl">Retorno neto estimado</span><span class="rn">~$2.000.000 / mes</span></div>
  </div>
...
<!-- 6 · Precio + CTA -->
<section class="page">
  <div class="kick">Empezá hoy</div>
  <div class="pricewrap">
    <div class="priceline">Desde $399.000<span style="font-size:24px;color:var(--ink3)"> / mes</span></div>
    <div class="pricesub">Sin permanencia. Listo en menos de una semana.<br>
      El plan se elige según el tamaño de tu restaurante. Precios en COP, no incluyen IVA.</div>
    <div class="promo">★ Pago anual: hasta 20% de descuento — promo de lanzamiento</div>
  </div>
  <div class="ctabox">
    <div class="glow"></div>
    <div class="ct">Agendá una demo gratuita.</div>
