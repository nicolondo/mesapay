"""
Propuesta para Ejecutivo Comercial Independiente EN EQUIPO de MESAPAY.

Salida: /Users/nicolas/Documents/APPS/MESAPAY/docs/Propuesta-Comercial-Equipo-Independiente-MESAPAY.pdf

Es el comercial SIN sueldo que trabaja bajo un líder comercial que TAMPOCO tiene
sueldo. Mismos porcentajes que el comercial CON sueldo (75% apertura + 10%
recurrente = 185%), pero sin el básico. El 25% de apertura + 2,5% recurrente
restantes los gana el líder, pagados por MESAPAY (no se descuentan del comercial).

Modelo de compensación:
- 75% del primer mes de cada cuenta activada (apertura)
- 10% recurrente mensual del mes 2 al 12
- Total año 1 por cuenta = 185% del precio mensual = 1,85 mensualidades
- El líder recibe override 25% apertura + 2,5% recurrente (paga MESAPAY)
"""
...
OUTPUT = "/Users/nicolas/Documents/APPS/MESAPAY/docs/Propuesta-Comercial-Equipo-Independiente-MESAPAY.pdf"
...
PCT_APERTURA = 0.75        # 75% del precio mes 1 (igual que el comercial con sueldo)
PCT_RECURRENTE = 0.10      # 10% mensual del mes 2 al 12
MESES_RECURRENTE = 11
TOTAL_ANO_1_PCT = PCT_APERTURA + PCT_RECURRENTE * MESES_RECURRENTE  # 185%

# Override que recibe el LÍDER (lo paga MESAPAY, no el comercial)
PCT_LIDER_APERTURA = 0.25
PCT_LIDER_RECURRENTE = 0.025

BASE_COMERCIAL_CON_SUELDO = 2_000_000  # solo para la tabla comparativa
...
style_td_green = ParagraphStyle("TDG", parent=style_td_rb, textColor=GREEN_OK)
...
def build():
    ...
    doc = BaseDocTemplate(
        ...
        title="Propuesta · Comercial Independiente en Equipo · MESAPAY",
        author="MESAPAY",
    )
    ...
    story.append(Paragraph("PROPUESTA · CORRETAJE MERCANTIL · EN EQUIPO", style_eyebrow))
    story.append(Paragraph("Propuesta de colaboración", style_title))
    story.append(Paragraph(
        "Ejecutivo Comercial Independiente (en equipo) · bajo un líder comercial · "
        "Modalidad: corretaje mercantil",
        style_subtitle,
    ))
    ...
    stats = Table([[
        ...
        [Paragraph("1,85", stat_big), Paragraph("mensualidades del plan<br/>por cuenta en año 1", stat_label)],
    ...
    story.append(Paragraph(
        ...
        "MESAPAY como corretaje mercantil, integrados a un <b>equipo bajo un líder "
        "comercial independiente</b> (que tampoco tiene sueldo). Por cada cuenta que "
        "actives recibís el <b>75% del primer mes</b> del plan que contrate el restaurante "
        "y el <b>10% mensual del mes 2 al 12</b> mientras la cuenta siga al día. Cada "
        "cuenta te paga <b>1,85 mensualidades del plan</b> en su primer año — los "
        "<b>mismos porcentajes que un comercial con sueldo, pero sin sueldo</b> y con "
        "total libertad. El 25% de apertura y el 2,5% recurrente restantes los gana tu "
        "líder, <b>pagados por MESAPAY</b> (no se descuentan de tu comisión).",
        style_body,
    ))
    ...
    story.append(Paragraph(
        "Dos componentes acumulables, <b>sin techo</b> y <b>sin sueldo</b>:",
        style_body,
    ))
    story.append(Paragraph(
        "<b>Mes 1 — Apertura:</b> el <b>75%</b> del precio mensual del plan que contrate "
    ...
    data = [
        [Paragraph("Plan", style_th),
         ...
         Paragraph("Apertura (mes 1)<br/>75% del precio", style_th),
    ...
    story.append(Paragraph(
        "Cada cuenta = 1,85 mensualidades del plan en el año 1 "
        "(75% mes 1 + 10% × 11 mes 2-12). Cifras en COP sobre suscripción de lista.",
        style_caption,
    ))
    ...
    story.append(Paragraph(
        "<b>Comisión anual = 185% del precio mensual de lista</b>, pagada toda junta.",
        style_bullet, bulletText="·",
    ))
    story.append(Spacer(1, 4))
    data = [
        [Paragraph("Plan", style_th),
         Paragraph("Mes 1 anual<br/>(185% del precio)", style_th),
         Paragraph("Mes 1 mensual<br/>(solo 75% apertura)", style_th)],
    ]
    for nombre, precio, _ in MIX:
    ...
    story.append(Paragraph(
        "Cerrar un Premium anual te deja $2,96M en el bolsillo en el mes 1.",
        style_caption,
    ))

    story.append(PageBreak())

    # === 3. Tu lugar en el equipo ===
    story.append(Paragraph("3. Tu lugar en el equipo", style_section))
    story.append(Paragraph(
        "Trabajás dentro del equipo de un <b>líder comercial independiente</b> (que tampoco "
        "tiene sueldo). El reparto es simple y MESAPAY le paga a cada uno por separado:",
        style_body,
    ))
    story.append(Paragraph(
        "<b>Vos conservás 75% de apertura + 10% recurrente</b> — 1,85 mensualidades por "
        "cuenta. Es tu comisión completa.",
        style_bullet, bulletText="·",
    ))
    story.append(Paragraph(
        "<b>Tu líder gana un override de 25% de apertura + 2,5% recurrente</b> sobre tus "
        "cuentas, <b>pagado directamente por MESAPAY</b>. No sale de tu bolsillo: MESAPAY "
        "paga el 100% del mes 1 repartido (75% vos / 25% líder).",
        style_bullet, bulletText="·",
    ))
    story.append(Paragraph(
        "<b>A cambio, tu líder te respalda:</b> capacitación, acompañamiento en cierres, "
        "leads y soporte operativo. No vendés solo.",
        style_bullet, bulletText="·",
    ))

    story.append(Paragraph("3.1 Cómo te comparás con los otros esquemas", style_h3))
    data = [
        [Paragraph("Rol", style_th),
         Paragraph("Sueldo", style_th),
         Paragraph("Apertura<br/>(mes 1)", style_th),
         Paragraph("Recurrente<br/>(mes 2-12)", style_th),
         Paragraph("Total<br/>año 1", style_th)],
        [Paragraph("Comercial con sueldo", style_td_b),
         Paragraph(cop(BASE_COMERCIAL_CON_SUELDO) + "/mes", style_td_r),
         Paragraph("75%", style_td_r),
         Paragraph("10%", style_td_r),
         Paragraph("185%", style_td_rb)],
        [Paragraph("Comercial independiente (solo)", style_td_b),
         Paragraph("—", style_td_r),
         Paragraph("100%", style_td_r),
         Paragraph("10%", style_td_r),
         Paragraph("210%", style_td_rb)],
        [Paragraph("<b>Comercial en equipo (vos)</b>", style_td_b),
         Paragraph("—", style_td_r),
         Paragraph("75%", style_td_r),
         Paragraph("10%", style_td_r),
         Paragraph("185%", style_td_green)],
    ]
    story.append(styled_table(data, [5.6 * cm, 3.2 * cm, 2.7 * cm, 3.0 * cm, 2.5 * cm]))
    story.append(Paragraph(
        "Ganás <b>los mismos porcentajes que un comercial con sueldo</b>, pero sin el "
        "básico y con libertad total. Frente al independiente que va solo, cedés 25 puntos "
        "de apertura a cambio del respaldo, los leads y la capacitación de tu líder.",
        style_caption,
    ))

    # === 4. Escenarios ===
    story.append(Paragraph("4. Escenarios de ingreso", style_section))
    story.append(Paragraph(
    ...
    # === 5. Reglas claras ===
    story.append(Paragraph("5. Reglas claras (para que no haya sorpresas)", style_section))
    for r in [
        ...
        "<b>Base de cálculo:</b> los porcentajes (75% y 10%) se calculan sobre la suscripción mensual del local de lista (no sobre impuestos ni sobre el markup de pagos), salvo acuerdo escrito distinto.",
        "<b>Override del líder:</b> el 25% de apertura y el 2,5% recurrente del líder los paga MESAPAY directamente. <b>Nunca se descuentan de tu comisión.</b>",
        ...
        "<b>Comisión sobre anuales:</b> 185% del precio mensual de lista liquidado en el mes 1. Tu base de cálculo es siempre el precio de lista, no el precio con descuento.",
        ...
        "<b>Tus cuentas son tuyas:</b> las cuentas que abrís quedan asignadas a vos para efectos de comisión durante todo el primer año, aunque cambies de líder.",
    ...
    story.append(Paragraph("6. Naturaleza de la relación", style_section))
    story.append(Paragraph(
    ...
    for b in [
        "<b>Sin subordinación laboral, sin horario fijo, sin metas mínimas obligatorias.</b> Coordinás con tu líder, pero no hay vínculo de empleo.",
    ...
    # === 7. Por qué te conviene ===
    story.append(Paragraph("7. Por qué te conviene", style_section))
    for b in [
        "<b>Mismos porcentajes que un asalariado, sin sueldo:</b> 75% del primer mes + 10% mensual del mes 2 al 12, sin techo.",
        "<b>No vendés solo:</b> tu líder te da leads, capacitación y acompañamiento en cierres — y su override lo paga MESAPAY, no vos.",
        "<b>Renta recurrente:</b> cada cuenta te paga 11 meses adicionales después del cierre, mientras siga al día.",
        "<b>Libertad total:</b> sin subordinación, sin horario, sin meta mínima.",
    ...
    for i, p in enumerate([
        "Conversación con tu líder y la dirección comercial para definir territorio y alcance.",
        "Firma del contrato de corretaje mercantil.",
        "Acceso a la demo, lista de precios, herramientas comerciales y material de ventas.",
        "Arranque en tu pipeline con el respaldo de tu equipo.",
    ], 1):
    ...
    firma = Table([
        [Paragraph("_______________________________", style_body_l),
         Paragraph("_______________________________", style_body_l)],
        [Paragraph("<b>Acepta — Comercial Independiente (Equipo)</b>", style_body_l),
