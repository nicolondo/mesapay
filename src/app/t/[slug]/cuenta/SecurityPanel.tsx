 ...
 *
 * Todo va contra `/api/tenant/<slug>/diner/*`: los dispositivos que se
 * cierran son los de la cuenta de ESTE comercio. Si la persona también está
 * registrada en otro restaurante MESAPAY, esa es otra cuenta y no se toca.
 */
export function SecurityPanel({
  ...
  const [busy, setBusy] = useState<
    null | "others" | "all" | "password" | "logout"
  >(null);
  ...
  async function closeSessions(keepCurrent: boolean) {
    ...
    setMsg(null);
    try {
      const res = await fetch(`/api/tenant/${slug}/diner/sessions`, {
        method: "DELETE",
      ...
      if (!keepCurrent) {
        ...
        window.location.href = `/t/${slug}/cuenta/entrar`;
        return;
      }
  ...
  async function signOut() {
    ...
    setErr(null);
    try {
      await fetch(`/api/tenant/${slug}/diner/logout`, { method: "POST" });
      window.location.href = `/t/${slug}/cuenta/entrar`;
    } catch {
      setErr(t("errGeneric"));
      setBusy(null);
  ...
  async function changePassword(e: React.FormEvent) {
    ...
    setMsg(null);
    try {
      const res = await fetch(`/api/tenant/${slug}/diner/password`, {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  return (
    <div className="rounded-2xl border border-hairline bg-paper p-5 space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          {busy === "logout" ? t("closing") : t("signOut")}
        </button>
        <button
          type="button"
