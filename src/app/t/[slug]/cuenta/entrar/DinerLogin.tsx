...
/**
 * Formulario de ingreso del comensal en UN comercio.
 *
 * Todo el fetch va contra `/api/tenant/<slug>/diner/*`: el restaurante
 * viaja en la ruta, nunca en el cuerpo. Si lo mandara el cliente, cualquiera
 * podría pedir un enlace de acceso "por" otro local.
 */
export function DinerLogin({
  slug,
  restaurantName,
}: {
  slug: string;
  ...
  return (
    <Suspense fallback={null}>
      <Inner slug={slug} restaurantName={restaurantName} />
    </Suspense>
  );
}
...
function Inner({
  ...
  const callbackUrl = search.get("callbackUrl") ?? `/t/${slug}/cuenta`;
  ...
  async function onSubmitPassword(e: React.FormEvent) {
    ...
    setErr(null);
    try {
      const res = await fetch(`/api/tenant/${slug}/diner/login`, {
        method: "POST",
  ...
  async function onSubmitMagic(e: React.FormEvent) {
    ...
    try {
      ...
      await fetch(`/api/tenant/${slug}/diner/magic-link`, {
        method: "POST",
        headers: { "content-type": "application/json" },
