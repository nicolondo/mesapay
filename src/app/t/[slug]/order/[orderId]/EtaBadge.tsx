...
export function EtaBadge({ etaAtISO }: { etaAtISO: string }) {
  ...
  const label =
    diffMin <= 1 ? "Casi listo" : diffMin <= 3 ? "En unos minutos" : `~${diffMin} min`;
...
export function OrderEta({ etaAtISO }: { etaAtISO: string }) {
  ...
  const headline =
    diffMin <= 1
      ...
      : diffMin <= 3
        ? "Tu pedido sale en unos minutos"
        : `Tu pedido sale en ~${diffMin} min`;
  const sub =
    diffMin <= 1
      ? "Ya casi te lo llevamos."
      : "Puede cambiar si la cocina va más rápido o se llena.";
  return (
