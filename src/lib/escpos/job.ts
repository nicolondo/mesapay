 ...
 *
 * Hay tres documentos y va a haber más (la tirilla de cierre de turno es
 * la próxima). Quién decide cuál es NO es `PrintJob.kind` sino la FORMA
 * del sobre: `{ v, ticket }` es una comanda, `{ v, invoice }` una
 * factura, `{ v, prebill }` una precuenta. Es a propósito — `kind` es un String libre justamente para que
...
import { parsePrebillPayload, renderPrebill } from "./prebill";
...
export function renderPrintJobPayload(raw: unknown): Buffer | null {
  ...
  const prebill = parsePrebillPayload(raw);
  if (prebill) return renderPrebill(prebill);
  return null;
}
